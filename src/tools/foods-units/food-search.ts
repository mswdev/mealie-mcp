...
const inputSchema = {
  search: z.string().optional().describe("Full-text search filter (e.g. a food name)"),
...
/**
 * Handles food_search: lists ingredient foods with pagination.
 *
 ...
 */
export async function foodSearchHandler(
  client: SearchClient,
  args: SearchArgs,
): Promise<CallToolResult> {
  try {
    const page = await client.getPaginated<FoodDetail>("/api/foods", {
      ...args,
      perPage: args.perPage ?? DEFAULT_PER_PAGE,
    });
    return jsonResult(toConcise(page));
  } catch (error) {
    return errorResult(error, "food_search", "Failed to search foods");
  }
}
...
/**
 * Registers the food_search tool on the server.
 *
 * @param server - The McpServer to register on
 * @param client - The MealieClient used by the handler
 */
export function registerFoodSearch(server: McpServer, client: MealieClient): void {
  server.registerTool(
    "food_search",
    {
      title: "Search Foods",
      description:
        "Search and filter ingredient foods with pagination. Returns concise items (id, name, labelId). Use to resolve a food name to its id for recipes/shopping items.",
      inputSchema,
      annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: true },
    },
    (args) => foodSearchHandler(client, args),
  );
}
